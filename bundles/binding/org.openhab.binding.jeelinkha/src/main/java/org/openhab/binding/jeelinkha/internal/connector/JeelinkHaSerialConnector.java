 ...
 * @since 1.2.0
 */
public class JeelinkHaSerialConnector implements JeelinkHaConnectorInterface {

	private static final Logger logger = LoggerFactory
			.getLogger(JeelinkHaSerialConnector.class);

	private static List<JeelinkHaEventListener> _listeners = new ArrayList<JeelinkHaEventListener>();
	...
	public JeelinkHaSerialConnector() {
	}
					...
					for (int i = 0; i < len; i++) {
						...
						if (start_found == false && tmpData[i] > 0) {
							...
							if (index == msgLen) {
								...
								JeelinkHaMessageReceivedEvent event = new JeelinkHaMessageReceivedEvent(
										this);

								try {
									Iterator<JeelinkHaEventListener> iterator = _listeners
											.iterator();

									while (iterator.hasNext()) {
										((JeelinkHaEventListener) iterator.next())
